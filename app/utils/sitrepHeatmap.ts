...
/** Highest tier (dark red) opacity cap on the map. Other tiers scale proportionally. */
const HEATMAP_MAX_OPACITY = 1

const HEATMAP_TIER_DEFINITIONS = [
  { ratio: 1, color: [139, 0, 0] as const, opacityRatio: 1 },
  { ratio: 0.8, color: [255, 0, 0] as const, opacityRatio: 0.8 },
  { ratio: 0.6, color: [234, 88, 12] as const, opacityRatio: 0.6 },
  { ratio: 0.4, color: [255, 140, 0] as const, opacityRatio: 0.4 },
  { ratio: 0.1, color: [255, 240, 0] as const, opacityRatio: 0.1 },
] as const
...
function tierOpacity(opacityRatio: number): number {
  return HEATMAP_MAX_OPACITY * opacityRatio
}

/** Blob radius in px: diameter ≈ two grid cells. */
function getHeatmapBlobRadius(width: number, height: number): number {
  const gridWidth = RASTER_MAP_GRID_BOUNDS.right - RASTER_MAP_GRID_BOUNDS.left
  const gridHeight = RASTER_MAP_GRID_BOUNDS.bottom - RASTER_MAP_GRID_BOUNDS.top
  const cellWidth = (gridWidth / RASTER_COLUMNS.length) * width
  const cellHeight = (gridHeight / RASTER_ROWS.length) * height
...
export function getHeatmapTiers(topThreshold: number): HeatmapTier[] {
  const scaledTiers = HEATMAP_TIER_DEFINITIONS.map(definition => ({
    ...
    opacity: tierOpacity(definition.opacityRatio),
  }))
  ...
  return [
    ...scaledTiers,
    {
      ...
      opacity: tierOpacity(HEATMAP_ZERO_TIER.opacityRatio),
    },
  ]
}
...
export function drawIncidentHeatmap(
  ...
  const radius = getHeatmapBlobRadius(width, height)
